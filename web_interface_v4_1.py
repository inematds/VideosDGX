#!/usr/bin/env python3
"""
Interface Web para Geração de Vídeos - v4.1 Multi-Modelo + Image-to-Video
Suporta: LTX-2 19B | Wan 2.2 5B | Wan 2.2 14B MoE
I2V: LTX-2 (LTXVImgToVideoConditionOnly) | Wan 2.2 5B (WanImageToVideo)
Acesse em: http://localhost:7861
"""
...
app = FastAPI(title="DGX Video Studio v4.1")
...
JOBS_FILE      = Path("/tmp/dgx_jobs_v4_1.json")
...
SCRIPTS = {
    ...
    "ltx2_i2v":      BASE_DIR / "gerar_video_ltx2_i2v.py",
...
def build_cmd(job_id: str, req):
    ...
    model_key = req.model
    if has_image and req.model in ("ltx2", "wan22_5b"):
...
<!-- SIDEBAR -->
<div class="sidebar">
  <div class="logo">DGX <span class="name">Video Studio</span> <span class="ver">v4.1</span></div>
  ...
  <!-- Image upload (só aparece para LTX-2 e Wan 2.2 5B) -->
...
function selectModel(model, el) {
  ...
  const i2vSupport = ['ltx2', 'wan22_5b'].includes(model);
...
if __name__ == "__main__":
    print("=" * 60)
    print("  DGX Video Studio v4.1 - Multi-Model + Image-to-Video")
    print("=" * 60)
    print()
